...
// FloatingScore can move itself on screen following a B�zier curve
public class FScore : MonoBehaviour
{
    [Header("Set Dynamically")]
    public eFSStateG state = eFSStateG.idle;
    [SerializeField]
...
public GameObject reportFinishTo = null;
    ...
    public void Init(List<Vector2> ePts, float eTimeS = 0, float eTimeD = 1)
    {
        ...
        timeStart = eTimeS;
        timeDuration = eTimeD;
        state = eFSStateG.pre; // Set it to the pre state, ready to start moving
    }
    ...
    void Update()
    {
        ...
        if (state == eFSStateG.idle) return;
